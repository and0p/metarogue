...
public class GUIUpdater {
    ...
    public static Box offsetAbsoluteElement(GUIElement e, Box parentBox) {
        // Offset the top-left vector of the new box
        Vector2d topLeft = new Vector2d(parentBox.getCorner(0).getX()+ e.padding[3] + e.bordersize,
                parentBox.getCorner(0).getY() + e.padding[0] + e.bordersize);
        // Offset the bottom-right vector of the new box
        Vector2d bottomRight = new Vector2d(parentBox.getCorner(2).getX() - e.padding[1] - e.bordersize,
                parentBox.getCorner(2).getY() - e.padding[2] - e.bordersize);
        Box box = new Box(topLeft, bottomRight);
        return box;
    }
